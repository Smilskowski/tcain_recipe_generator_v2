...
function computeCraftResult(
  components: number[],
  seedIn: number | string | undefined,
  legacyCalc: () => number
): number {
  const pg = loadPGBag();
  if (!pg) return legacyCalc();
  ...
  calculate(components: number[], seed?: number | string) {
    ...
    const legacyCalc = (): number => {
      ...
      for (let poolWeight of poolWeights) {
        if (poolWeight.weight <= 0) continue;
        ...
        if (n > 34) {
          qualityMin = 4; qualityMax = 4;
        } else if (n > 30) {
          qualityMin = 3; qualityMax = 4;
        } else if (n > 26) {
          qualityMin = 2; qualityMax = 4;
        } else if (n > 22) {
          qualityMin = 1; qualityMax = 4;
        } else if (n > 18) {
          qualityMin = 1; qualityMax = 3;
        } else if (n > 14) {
          qualityMin = 1; qualityMax = 2;
        } else if (n > 8) {
          qualityMin = 0; qualityMax = 2;
        }
        ...
        for (let item of pool.items) {
          ...
          if (quality > qualityMax) continue;
      ...
      for (let i = 0; i < itemWeights.length; i++) {
        if (target < itemWeights[i]) return i;
